...
- 地区画像：补齐美国、英国、德国、日本、韩国、新加坡、香港、台湾、印度、加拿大和俄罗斯的常用浏览器画像模板。
- 硬件画像：逻辑处理器快捷候选收敛为 8 / 12 / 16 / 24 / 32，同时保留自定义输入。
- 代理定位：热门地区备用选项和精确 IANA 时区解析覆盖同步完善。

### 1.9.0 · 2026-09-22

- 实例级远程调试：默认关闭，普通浏览器启动不再自动注入远程调试端口；自动化 Demo 按需显式开启。
- 实例配置安全：修复运行中实例删除、配置编辑和用户数据根目录迁移时的状态保护。
- 默认书签：指纹检测页不再固定出现在书签中，用户可以自行管理检测入口和启动行为。
- 数据一致性：SQLite、配置文件、备份导入和实例复制统一保存远程调试开关。
